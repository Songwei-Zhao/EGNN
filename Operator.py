...
#函数
def F1(feature_list, norm_fea_inf, x, alpha =0.2):
    input_feature = []
    # min = float('inf')
    for i in range(feature_list[0].shape[0]):
        ...
        if x[i] == 0:
            ...
            for j in range(int(x[i])):
                fea += alpha * feature_list[j][i].unsqueeze(0) + (1-alpha) * feature_list[0][i].unsqueeze(0)
            fea = fea / (int(x[i]))
        input_feature.append(fea)
    input_feature = torch.cat(input_feature, dim=0)
    dist = (input_feature - norm_fea_inf).norm(2, 1)
    s = sum(dist)
    return input_feature, s

def F2(feature_list, norm_fea_inf, x, args, n_class, labels, idx_train, idx_val, idx_test, alpha=0.1):
    input_feature = []
    # min = float('inf')
    for i in range(feature_list[0].shape[0]):
        # hop = hops[i].int().item()
        if x[i] == 0:
            ...
            fea = 0
            for j in range(int(x[i])):
                fea += (1-alpha) * feature_list[j+1][i].unsqueeze(0) + alpha * feature_list[0][i].unsqueeze(0)
                # fea += (1 - alpha - j / int(x[i])) * feature_list[j][i].unsqueeze(0) + alpha * feature_list[0][i].unsqueeze(0)
            fea = fea / (int(x[i]))
        input_feature.append(fea)
    input_feature = torch.cat(input_feature, dim=0)
    ...
    record = {}
    model = DNN(feature_list[0].shape[1], args.hidden, n_class, args.dropout).to(device)
    optimizer = optim.Adam(model.parameters(), lr=args.lr, weight_decay=5e-4)
    for epoch in range(100):
        acc_val = train(epoch, model, input_feature, record, idx_train, idx_val, idx_test, labels.to(device), optimizer)
    return acc_val, input_feature


def F3(feature_list, norm_fea_inf, x, args, n_class, labels, idx_train, idx_val, idx_test, alpha=0.1):
    input_feature = []
    # min = float('inf')
    for i in range(feature_list[0].shape[0]):
        # hop = hops[i].int().item()
    ...
    acc_test = accuracy(input_feature[idx_test], labels[idx_test])
    return acc_test, input_feature